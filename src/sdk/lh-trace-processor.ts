...
class TraceProcessor {
  ...
  static filteredTraceSort(traceEvents, filter) {
    ...
    indices.sort(
      (indexA, indexB) => traceEvents[indexA].ts - traceEvents[indexB].ts
    );
  ...
  static getRiskToResponsiveness(
    ...
    const ret = this.getMainThreadTopLevelEventDurations(
      events,
      startTime,
      endTime
    );
  ...
  static computeTraceOfTab(trace, options) {
    ...
    const frames = keyEvents
      ...
      .filter(
        /** @return {data is {frame: string, url: string}} */ data => {
